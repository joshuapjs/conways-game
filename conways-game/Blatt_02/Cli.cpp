...
Cli::Cli() : printOn(false), delayTime(100), stabilityCheck(false), world(nullptr) {}
  ...
  while (std::getline(std::cin, input)) {
    ...
    if (std::regex_match(input, std::regex(R"(\s*quit\s*)")) ||
      ...
      if (std::regex_match(input, match, unix_path_regex)) {
        ...
        if (world->load(path)) {
          std::cout << "Following path for loading was specified: " << path << std::endl;
        } else {
      ...
      if (std::regex_match(input, match, unix_path_regex)) {
        ...
        if (!world) {
          std::cout << "Error: No world created. Use 'create' or 'load' first." << std::endl;
        } else if (world->save(path)) {
          std::cout << "Following path for saving was specified: " << path << std::endl;
        } else {
     ...
     //set delay
      ...
      std::regex command_delay(R"(\s*delay\s+([0-9]+)\s*)");
      std::smatch match;
    ...
    // Stability check
    } else if (std::regex_match(input, std::regex(R"(\s*stability\s+[01]\s*)"))) {
      std::regex command_stability(R"(\s*stability\s+([01])\s*)");
      std::smatch match;

      if (std::regex_match(input, match, command_stability)) {
        stabilityCheck = std::stoi(match[1]);
        std::cout << "Stability check " << (stabilityCheck ? "enabled" : "disabled") << std::endl;
        input = "";
        std::cout << "CGOL> ";
      }
     ...
     // Run simulation
      ...
      std::regex command_run(R"(\s*run\s+([0-9]+)\s*)");
      std::smatch match;
        ...
        if (!world) {
          ...
          for (int i = 0; i < generations; ++i) {
            ...
            if (stabilityCheck && world->isStable()) {
              std::cout << "Stability reached after " << (i + 1) << " generations" << std::endl;
              break;
            }
          }
          ...
          std::cout << "Time for " << generations << " generations: " << duration.count() << " ms" << std::endl;
        }
        input = "";
        std::cout << "CGOL> ";
      }
     ...
     // Set cell state 2D 
    } else if (std::regex_match(input, std::regex(R"(\s*set\s+[0-9]+\s+[0-9]+\s+[01]\s*)"))) {
      std::regex command_set_2d(R"(\s*set\s+([0-9]+)\s+([0-9]+)\s+([01])\s*)");
      std::smatch match;
      ...
      if (std::regex_match(input, match, command_set_2d)) {
        ...
        if (!world) {
          ...
          world->set(x, y, state);
          std::cout << "Cell at (" << x << ", " << y << ") set to " << (state ? "alive" : "dead") << std::endl;
        }
        input = "";
        std::cout << "CGOL> ";
      }
      // Set cell state 1D
    } else if (std::regex_match(input, std::regex(R"(\s*set\s+[0-9]+\s+[01]\s*)"))) {
      std::regex command_set_1d(R"(\s*set\s+([0-9]+)\s+([01])\s*)");
      std::smatch match;
      ...
      if (std::regex_match(input, match, command_set_1d)) {
        ...
        if (!world) {
          ...
          int y = pos / cols;
          world->set(x, y, state);
          std::cout << "Cell at position " << pos << " set to " << (state ? "alive" : "dead") << std::endl;
        }
        input = "";
        std::cout << "CGOL> ";
      }
      // Get cell state 2D
    } else if (std::regex_match(input, std::regex(R"(\s*get\s+[0-9]+\s+[0-9]+\s*)"))) {
      std::regex command_get_2d(R"(\s*get\s+([0-9]+)\s+([0-9]+)\s*)");
      std::smatch match;
      ...
      if (std::regex_match(input, match, command_get_2d)) {
        ...
        if (!world) {
          ...
          std::cout << "Cell at (" << x << ", " << y << ") - use world inspection" << std::endl;
        }
        input = "";
        std::cout << "CGOL> ";
      }
      ...
      std::regex command_get_1d(R"(\s*get\s+([0-9]+)\s*)");
      std::smatch match;
      ...
      if (std::regex_match(input, match, command_get_1d)) {
        ...
        if (!world) {
          ...
          std::cout << "Cell at position " << pos << " - use world inspection" << std::endl;
        }
        input = "";
        std::cout << "CGOL> ";
      }

    } else if (std::regex_match(input, std::regex(R"(\s*glider\s+[0-9]+\s+[0-9]+\s*)"))) {
      std::regex command_glider(R"(\s*glider\s+([0-9]+)\s+([0-9]+)\s*)");
      std::smatch match;
    ...
    // Toad pattern
    } else if (std::regex_match(input, std::regex(R"(\s*toad\s+[0-9]+\s+[0-9]+\s*)"))) {
      std::regex command_toad(R"(\s*toad\s+([0-9]+)\s+([0-9]+)\s*)");
      std::smatch match;
    ...
    // Beacon pattern
    } else if (std::regex_match(input, std::regex(R"(\s*beacon\s+[0-9]+\s+[0-9]+\s*)"))) {
      std::regex command_beacon(R"(\s*beacon\s+([0-9]+)\s+([0-9]+)\s*)");
      std::smatch match;
